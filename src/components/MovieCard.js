     ...
     const mv = this.props.movies.map((movie) => 
            ...
            <div className="badge badge-success badge-vote">{movie.vote_average}</div>
                {movie.poster_path == null && 
                    <Link to={`/movie-detail/${movie.id}`}><img src={noImage} className="card-img-top"/></Link>
                }
                {movie.poster_path != null && 
                    <Link to={`/movie-detail/${movie.id}`}><img src={`http://image.tmdb.org/t/p/w185//${movie.poster_path}`} className="card-img-top"/></Link>
